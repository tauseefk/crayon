//! Headless wgpu device + CPU readback — the enabler for automated renderer
//! tests: render a fixture document offscreen, read it back, probe pixels.

/// Headless device+queue: an adapter requested with `compatible_surface: None`
/// needs no window and no surface. Uses `downlevel_webgl2_defaults()` so tests
/// fail on the SAME capability ceiling as WebGL (2048 max texture dim, 4 bind
/// groups) rather than passing on a permissive native adapter and breaking
/// only on web.
pub fn headless_gpu() -> (wgpu::Device, wgpu::Queue) {
    ...
    .expect("headless_gpu: no wgpu adapter available on this machine");
    pollster::block_on(adapter.request_device(&wgpu::DeviceDescriptor {
        label: Some("Headless Test Device"),
        required_limits: wgpu::Limits::downlevel_webgl2_defaults(),
        ..Default::default()
    }))
    .expect("headless_gpu: adapter refused a device with webgl2 downlevel limits")
}

/// Map a render texture back to CPU RGBA8.
/// Handles the 256-byte bytes-per-row alignment wgpu requires for texture→buffer copies,
/// then strips the padding. The texture must have `COPY_SRC`.
...
";
    ...
    /// `Rgba8Unorm` keeps assertions byte-exact, no srgb conversion.
    #[test]
