//! In-code document + pixel fixtures — deterministic inputs with no PNG
//! assets on disk.
...
/// One artboard, one blank layer.
pub fn doc_single_layer() -> Document {
    Document::default_document()
}

/// Two artboards at distinct world positions, one blank layer each — the
/// in-code analogue of `two-boards.json`, for placement tests.
...
/// Premultiplied solid fill of `size`, for hydrating layer textures in tests.
...
mod tests {
    ...
    fn solid_layer_pixels_is_premultiplied() {
        let pixels = solid_layer_pixels((3, 2), [255, 255, 255, 128]);
        assert_eq!(pixels.len(), 3 * 2 * 4);
        // Premultiplied: color channels scaled by alpha.
