//! Artboard-context handler (§3.3): cmd+drag moves the artboard; a plain
//! click inside the artboard routes down — select the top layer and start
//! drawing (`SelectLayer` is queued before `StrokeStart`, so the stroke
//! resolves against the updated selection).
    ...
    fn handle(&mut self, ctx: SelectionCtx, action: &InputAction, env: &DispatchEnv) -> Handled {
        ...
        match *action {
            InputAction::PointerDown { screen } => {
                ...
                if !artboard.contains(world) {
                    return Handled::No;
                }
                // click-to-select-layer routing; an artboard with no layers
                // still claims the click (nothing to draw on)
